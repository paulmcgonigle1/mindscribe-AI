...
logger = logging.getLogger(__name__)  # Use a logger for your current module
...
class CreateImprovementWithTasksAndMessage(APIView):
    ...
    def get(self, request):
        ...
        return Response({"tasks": mental_health_tasks})
...
class MessageOfDayView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        user = request.user
        today = timezone.now().date()
        message = self.create_message_of_the_day(user)

        print(f"Generated Message: {message}")  # Debugging output
        return Response({"message": message})

    def create_message_of_the_day(self, user):
        ...
        if message:
            print(f"Received message from LLM: {message}")  # Debugging output
            user_improvement, created = UserImprovement.objects.get_or_create(
                user=user,
                date=today,
                defaults={
